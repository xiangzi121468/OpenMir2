...
function DecrypStr(Src, Key: string): string; // ????????????
var
  KeyLen: Integer;
  KeyPos: Integer;
  offset: Integer;
...
{$ENDIF}
  ...
  ConsoleDebug(Format('??????????????:%s', [g_MirStartupInfo.sServerKey]));
  ...
  begin
    ConsoleDebug(Format('???????????????:%s,???????????????,?????????????', [WIL.ResDir]));
  ...
  begin
    ...
    ConsoleDebug(Format('??????????:%d', [g_MirStartupInfo.nLocalMiniPort]));
  end;
...
{$IFEND SERIESSKILL}
  ...
  Self.Color := clblack; // 20200703LOGO?????
  // 20200703????????
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if g_MovingItem.Item.S.reserve[3] in [100 .. 249] then     //20200927??????????
        begin
...
begin
  if mrOk = FrmDlg.DMessageDlg('???????????????????', [mbOk, mbCancel]) then
  begin
    AppLogoutEx;
  end;
end;
...
begin
  ...
  DScreen.AddChatBoardString('[????????' + g_sServerName + '_Images' +
            ...
            begin
              ...
              try
                ...
                begin
                  g_MySelf.m_nTagX := 0;
                  g_MySelf.m_nTagY := 0;
                  DScreen.AddChatBoardString('?????????????????', GetRGB(5), clWhite);
                end;
              finally
                g_PathBusy := False;
              end;
      ...
      begin
        ...
        begin
          ...
          begin
            FrmDlg.m_BlockList.Delete(wc);
            FrmDlg.DFrdSave();
            DScreen.AddChatBoardString('???????' + g_FocusCret.m_sUserName +
              '??????????', GetRGB(219), clWhite);
          end
          else
          begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            g_nTargetX := -1;
            g_APGoBack := False;
            DScreen.AddChatBoardString('[???] ?????????...', clWhite, clRed);
            SaveWayPoint;
      ...
      begin
        ...
        begin
          ...
          if bHint then
            ...
            DScreen.AddSysMsgCenter('????????????', clLime, clblack, 10);
          bEatOK := False;
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      if bidx > -1 then
        ...
        // DScreen.AddChatBoardString('???' + g_sRenewBooks[g_gnProtectPercent[6]] + '???????', clWhite, clBlue);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          bSH := True;
          if bHint then
            // DScreen.AddChatBoardString(Format('???[%s]???????????', [g_MySelf.m_HeroObject.m_sUserName]), clWhite, clBlue);
            DScreen.AddSysMsgCenter(Format('???[%s]???????????',
...
procedure TfrmMain.AutoUnBindItem(nType: Integer; sItem: string);  //20201003???BUG
var
  i, n, idx: Integer;
              ...
              try
                ...
                begin
                  ...
                  DScreen.AddChatBoardString
                    (Format('????????????(%d:%d)???????????????????', [g_MySelf.m_nTagX,
                    g_MySelf.m_nTagY]), GetRGB(5), clWhite);
                end
                else
                begin
                  TimerAutoMove.Enabled := False;
                  DScreen.AddChatBoardString(Format('???????????(%d:%d)???????',
...
begin
  ...
  begin
    ConsoleDebug('InitWMImagesLib????????');
    DScreen.Initialize;
    ConsoleDebug('DScreen????????');
    FrmDlg.Initialize(True);
    ConsoleDebug('FrmDlg????????');
    g_PlayScene.Initialize;
    ConsoleDebug('PlayScene????????');
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                DScreen.AddSysMsgCenter(Format('???????????????????????',
                  [g_UseItems[i].S.Name]), clLime, clblack, 10);
              end;
            end;
          end;
...
procedure TfrmMain.SendNewAccount(ue: TUserEntryA); // ???????
var
  msg: TDefaultMessage;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            // ??????
            DScreen.AddChatBoardString(Format('???? %s ???????????????', [sM]),
              clBlue, clWhite);
            Exit;
          end;
          X := StrToInt(sx);
    ...
    begin
      if CompareText(rstr, '@@buildguildnow') = 0 then
        ...
        FrmDlg.DMessageDlg('???????????????????(1~1000)??', [mbOk, mbAbort])
...
begin
    ...
    case msg.ident of
      ...
      SM_NEWCHR_FAIL:
        begin
          case msg.Recog of
            0:
              FrmDlg.DMessageDlg('[???????] ?????????????????????? ??????? = 0', [mbOk]);
            2:
              FrmDlg.DMessageDlg('[???????] ???????????????????????? ??????? = 2', [mbOk]);
            3:
              FrmDlg.DMessageDlg('[???????] ???????????????????? ??????? = 3', [mbOk]);
            4:
              FrmDlg.DMessageDlg('[???????] ??????????????? ??????? = 4', [mbOk]);
          else
            FrmDlg.DMessageDlg('[???????] ??????????????????', [mbOk]);
          end;
        end;
  ...
  case msg.ident of
    ...
    SM_SecretProperty:
      begin
        FrmDlg.DBSP.OnbtnState := tnor; // ASP??????
    ...
    SM_ExchangeItem:
      begin
        FrmDlg.DBTIbtn1.OnbtnState := tnor; // ASP??????
    ...
    SM_DETECTITEM_FALI:
      begin
        ...
        case msg.series of
          00:
            begin
              DScreen.AddChatBoardString('[?????????????????????????????????]', clWhite,
                GetRGB($FC));
            end;
          01:
            begin
          ...
          09:
            DScreen.AddChatBoardString('[??????????????]', clWhite, GetRGB($FC));
          10:
            DScreen.AddChatBoardString('[????????????????????????????????]', clWhite,
              GetRGB($FC));
          11:
            DScreen.AddChatBoardString('[??????????????????]', clWhite, GetRGB($38));
          12:
            DScreen.AddChatBoardString('[???????????????????????????]', clWhite,
              GetRGB($FC));
    ...
    SM_QUERYCHANGEHERO_FALI:
      begin
        ...
        FrmDlg.DListBox_Hero.ChangingHero := False; // ASP??????
        if msg.Recog = 0 then
    ...
    SM_SENDHEROS:
      begin
        ...
        FrmDlg.DListBox_Hero.Items.Clear; // ASP??????
      ...
      begin
        ...
        case msg.Recog of
          ...
          -2:
            FrmDlg.DMessageDlg('[???] ????????????????????????', [mbOk]);
          -3:
            FrmDlg.DMessageDlg('[???] ???ID??????????????????', [mbOk]);
          -4:
            FrmDlg.DMessageDlg('[???] ?????????????????????????????????', [mbOk]);
          -5:
            FrmDlg.DMessageDlg('[???] ????????????????????????', [mbOk]);
          -6:
            FrmDlg.DMessageDlg('[???] ?????????????????(1~150,000,000)', [mbOk]);
          -7:
            FrmDlg.DMessageDlg('[???] ??????????????????(1~8,000,000)', [mbOk]);
          -8:
            FrmDlg.DMessageDlg('[???] ?????????', [mbOk]);
          -9:
            FrmDlg.DMessageDlg(Format('[???] %s ??????????', [DecodeString(body)]
              ), [mbOk]);
          -10:
            FrmDlg.DMessageDlg('[???] ??????????????', [mbOk]);
          -11:
...
{$IF SERIESSKILL}
    ...
    SM_SERIESSKILLREADY:
      begin
        ...
        begin
          DScreen.AddChatBoardString('(???) ??????????????????????', GetRGB(219),
            clWhite);
          g_SndMgr.PlaySound('Wav\warpower-up.wav');
        end;
      end;
...
{$IFEND SERIESSKILL}
        ...
        begin
          ...
          if ShellExecute(Handle, 'open', PChar(string('Iexplore.exe')),
            ...
            ShellExecute(Handle, 'open', PChar(string(body)), nil, nil,
              SW_SHOWNORMAL);
          // Debugoutstrs('???????????WIN10??????????'+IntTostr(kkks));
        end;
      end;
    SM_QUERYITEMDLG:
      ...
      begin
        ...
        case msg.Recog of
          ...
          -7:
            FrmDlg.ShowMDlg(0, '', '[???]??????????????????????????');
          -8:
            FrmDlg.ShowMDlg(0, '', '[???]??????????NPC?????????????????');
          -9:
            FrmDlg.ShowMDlg(0, '', '[???]????????????????????????????');
          -10:
            FrmDlg.ShowMDlg(0, '', '[???]????????????????????????0~9999???');
          -11:
            FrmDlg.ShowMDlg(0, '', '[???]????????????????????????0~9999???');
          -12:
            FrmDlg.ShowMDlg(0, '', '[???]????????????????');
    ...
    SM_LOGON:
      begin
        ...
        begin
          DScreen.AddChatBoardString('????????????IP ???', GetRGB(219), clWhite)
        end
...
{$IFEND VIEWFOG}
        ...
        begin
          if not g_gcGeneral[3] or
            ...
            DScreen.AddSysMsgBottom
              (Format('????? +%d', [LongWord(Makelong(msg.param, msg.tag))]));
        end
        else
        begin
          if not g_gcGeneral[3] or
      ...
      begin
        ...
        begin
          ...
          begin
            if not g_gcGeneral[3] or
              (LongWord(Makelong(msg.param, msg.tag)) > g_MaxExpFilter) then
              DScreen.AddSysMsgBottom(Format('(???)????? +%d',
    ...
    SM_WINNIMBUSEXP:
      begin
        if msg.Recog > 0 then
          DScreen.AddSysMsgBottom(Format('???????? %d', [msg.Recog]));
      end;
    SM_HEROWINNIMBUSEXP:
      begin
        if msg.Recog > 0 then
    ...
    SM_WINIPEXP:
      begin
        ...
        if ipExp > 0 then
          DScreen.AddSysMsgBottom(Format('%d?????????????', [ipExp]));
...
{$ENDIF OPENCENTERMAG}
    ...
    SM_HEROEXCHGBAGITEM_FAIL:
      ...
      begin
        ...
        begin
          AddItemBag(g_WaitingUseItem.Item);
          DScreen.AddChatBoardString(g_WaitingUseItem.Item.S.Name + '??????????????',
            clWhite, clRed);
          g_WaitingUseItem.Item.S.Name := '';
        end;
      end;
    ...
    SM_HEROEAT_FAIL:
      begin
        ...
        case msg.series of
          1:
            DScreen.AddChatBoardString('[???] ??????????????????????', clRed,
              clWhite);
          2:
            DScreen.AddChatBoardString('[???] ????????????????????????', clRed,
              clWhite);
      ...
      begin
        ...
        begin
          ...
          begin
            if g_SellDlgItem.S.Overlap > 0 then
              g_sSellPriceStr := '??: ' +
                IntToStr(msg.Recog * g_SellDlgItem.Dura) + '???????'
            else
              g_sSellPriceStr := '??: ' + IntToStr(msg.Recog) + '???????';
          end
          else
          begin
            case msg.Recog of
          ...
          begin
            ...
            begin
              g_SellDlgItemSellWait.Item.Dura := g_SellDlgItemSellWait.Item.Dura
                - msg.param;
              DScreen.AddChatBoardString(Format('?????? %s %d??',
      ...
      begin
        case msg.param of // Market System..
          ...
          UMResult_OverSellCount:
            FrmDlg.DMessageDlg('[???]: ???????????????. ????????? ' +
              IntToStr(MARKET_MAX_SELL_COUNT) + ' ?????.', [mbOk]);
          UMResult_LessMoney:
            begin
              FrmDlg.LastestClickTime := GetTickCount;
              if g_SellDlgItemSellWait.Item.S.Name <> '' then
          ...
          UMResult_LessLevel:
            FrmDlg.DMessageDlg('[???]: ??? ' + IntToStr(MARKET_ALLOW_LEVEL) +
              ' ??????????????????.', [mbOk]);
...
begin
  ...
  begin
    if g_wAvailIDDay = 1 then
      ...
      FrmDlg.DMessageDlg('?????IP?????? ' + IntToStr(g_wAvailIDDay) + ' ??', [mbOk]);
  end
  else if g_wAvailIPDay > 0 then
  begin
    if g_wAvailIPDay = 1 then
      FrmDlg.DMessageDlg('?????IP???????????', [mbOk])
    else if g_wAvailIPDay <= 3 then
      FrmDlg.DMessageDlg('?????IP?????? ' + IntToStr(g_wAvailIPDay) + ' ??', [mbOk]);
  end
  else if g_wAvailIPHour > 0 then
  begin
    if g_wAvailIPHour <= 100 then
      FrmDlg.DMessageDlg('?????IP?????? ' + IntToStr(g_wAvailIPHour) +
        ' ??', [mbOk]);
  end
  else if g_wAvailIDHour > 0 then
  begin
    FrmDlg.DMessageDlg('?????ID?????? ' + IntToStr(g_wAvailIDHour) + ' ??',
      [mbOk]);;
  end;
...
begin
  ...
  begin
    ...
    {$IF Var_UI = Var_185}
      FrmDlg.DscStart.tag := FrmDlg.DscStart.WLib.Images
        [g_sDscStart0 + BYTE(FrmDlg.DscStart.Downed)].Height; // ASP??????
    {$IFEND}
...
begin
  ...
  // ???????? (40000-50000)
  if idx < 50000 then begin
    if g_ArmorEffectImages <> nil then
      Result := g_ArmorEffectImages.GetCachedImage(idx - 40000, ax, ay);
    Exit;
  end;
  // 配饰装备图标 (50000-60000): 戒指、靴子、腰带
  if idx < 60000 then begin
    if g_WAccessoryImages <> nil then
      Result := g_WAccessoryImages.GetCachedImage(idx - 50000, ax, ay);
    Exit;
  end;
end;
...
begin
  ...
  if idx < 50000 then begin
    if g_ArmorEffectImages <> nil then
      Result := g_ArmorEffectImages.Images[idx - 40000];
    Exit;
  end;
  // 配饰装备图标 (50000-60000): 戒指、靴子、腰带
  if idx < 60000 then begin
    if g_WAccessoryImages <> nil then
      Result := g_WAccessoryImages.Images[idx - 50000];
    Exit;
  end;
end;
...
begin
  ...
  begin
    ...
    else if (FileIdx >= 50) and (FileIdx < 64) then
    begin
      // ?????? Shape 50-63 (FileIdx 50-63) ?? NewHum.data
      if g_WHum4ImgImages <> nil then
        Result := g_WHum4ImgImages.GetCachedImage(HUMANFRAME * (Dress - 100) +
          nFrame, ax, ay);
    end
    else if FileIdx >= 64 then
    begin
      if g_WHum3ImgImages <> nil then
        Result := g_WHum3ImgImages.GetCachedImage(HUMANFRAME * (Dress - 128) +
          nFrame, ax, ay);
    end
    else
...
{$IFEND CONFIGTEST}
  ...
  // 20200726???????
  g_Button_DetectBox := ClientConf.Button_DetectBox; // ???????
  FrmDlg.DBDetectBox.Visible := g_Button_DetectBox; // ???????
  g_Button_Mission := ClientConf.Button_Mission; // ????
  FrmDlg.DBMissionOpen.Visible := g_Button_Mission; // ?????
  g_Button_Shop := ClientConf.Button_Shop; // ????
  FrmDlg.DButtonShop.Visible := g_Button_Shop; // ??????

  g_Button_Friend := ClientConf.Button_Friend;
  FrmDlg.DBotFriend.Visible := g_Button_Friend; // ??????
  ...
  g_ShowWinExpMode := ClientConf.ShowWinExpMode; // ?????????
  g_IgnoreStruck := ClientConf.IgnoreStruck; // ??????
...
begin
  ...
  begin
    ...
    begin
      DScreen.AddChatBoardString(Format('%s????', [sHumanName]), clWhite, clRed);
      Exit;
    end;
  end;
  g_boShowMemoLog := not g_boShowMemoLog;
...
begin
  ...
  if { not frmWebBrowser.Showing and } not g_DlgInitialize and
    ((g_MySelf = nil) or (mrOk = FrmDlg.DMessageDlg('???????????',
    [mbOk, mbCancel]))) then
  begin
    if g_MySelf <> nil then
    begin
...
begin
  ...
  begin
    ...
    // FrmDlg.DMBChat.UpdatePos(ChatBoardTop);      //ASP???
  end;
end;
...
begin
  ...
  case GetAutoPalyStation() of
    0:
      begin
        if not EatItemName('????') and not EatItemName('??????') and
          not EatItemName('????????') and not EatItemName('???????') then
          DScreen.AddChatBoardString('[???] ????????????,??????!!!', clWhite, clRed)
        else
          DScreen.AddChatBoardString('[???] ?????????!!!', clWhite, clRed);
        g_gcAss[0] := False;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            AP_findpath(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nTargetX,
              g_nTargetY);
            g_sAPStr := Format('[???] ????????(%d,%d)', [g_nTargetX, g_nTargetY]);
            g_nTargetX := -1;
          end
          else
          begin
...
begin
  ...
  begin
    ...
    begin
      ...
      // //DScreen.AddChatBoardString('[?????] ????' + IntToStr(cltime - svtime), GetRGB(219), clWhite);
  ...
  begin
    ...
    // DScreen.AddChatBoardString('????? get ready...', clWhite, clRed);
  end
  ...
  begin
    ...
    g_dwLatestSmiteWideHitTick2 := GetTickCount;
    DScreen.AddChatBoardString('[??????] ?????', clBlue, clWhite);
    ...
    try
      ...
      if g_GameCanvas.BeginScene then
        try
          ...
          // ATexture := FontManager.GetFont('????', 9, []).TextOut('???????? : '+IntToStr(pmc.WorkingSetSize div 1024 div 1024)+'MB  '+sCPU);
